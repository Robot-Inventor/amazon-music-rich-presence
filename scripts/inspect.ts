...
const getAmazonMusicTarget = async (): Promise<DebugTarget> => {
    ...
    if (!response.ok) throw new Error(`Remote debugging endpoint returned ${String(response.status)}.`);
...
const getChromePath = (): string => {
    const { LOCALAPPDATA, PROGRAMFILES } = Bun.env;
    const candidates = [
        `${PROGRAMFILES ?? ""}\\Google\\Chrome\\Application\\chrome.exe`,
        `${Bun.env["PROGRAMFILES(X86)"] ?? ""}\\Google\\Chrome\\Application\\chrome.exe`,
        `${LOCALAPPDATA ?? ""}\\Google\\Chrome\\Application\\chrome.exe`
    ];
    const chromePath = candidates.find((path) => path && Bun.file(path).size);
...
const openInChrome = async (chromePath: string, url: string): Promise<void> => {
    ...
    if (await subprocess.exited) throw new Error("Could not activate the Chrome address bar.");
};
...
try {
    ...
    url = `devtools://devtools/bundled/inspector.html?ws=127.0.0.1:${String(DEBUG_PORT)}/devtools/page/${target.id}`;
